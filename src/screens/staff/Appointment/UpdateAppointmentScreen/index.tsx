...
import StatusPickerComponent from '../components/StatusPicker';

const UpdateAppointmentScreen: React.FC<IUpdateAppointmentScreenProps> = (
  props,
) => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: 'white', paddingTop: 16 }}>
      <CurrentBookingContainer actionType={'update'} />
      <ScrollView
      ...
      {bookingType != 'membership' && (
        <BookingActionsComponent actionType={'update'} />
      )}
      <AlreadyBookedTimeComponent />
    </View>
  );
};
export interface IUpdateAppointmentScreenProps {}
